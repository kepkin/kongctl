...
class BaseResource(object):
    ...
    def get_by_id(self, id):
        ...
        if resource is None:
            url = self._build_resource_url('get_by_id', None, None, id=id)
...
class RouteResource(BaseResource):
    ...
    def short_formatter(self, resource):
        ...
        self.formatter.print_pair(resource['id'], ref.get_by_id(resource['service']['id'])['name'])

        ops = []
        for h in hosts:
            for p in paths:
                self.formatter.println(h + p, indent=1)
    ...
    def _build_resource_url(self, op, args, non_parsed, **kwargs):
        if op in {'list'} and args.service is not None:
...
class PluginResource(BaseResource):
    ...
    def short_formatter(self, resource):
        ...
        if resource.get('service_id'):
            ref = ServiceResource(self.http_client)
            service_name = ref._get(resource['service_id'])['name']

        self.formatter.print_header("{}: {} (service {})".format(resource['id'], resource['name']))
        self.formatter.print_obj(service_name, indent=1)
    ...
    def _build_resource_url(self, op, args, non_parsed, **kwargs):
        if op in 'list' and args.service is not None:
            return '{}/{}/{}/{}/'.format(args.server, 'services', args.service, self.resource_name)
        else:
            return super()._build_resource_url(op, args, non_parsed, **kwargs)
